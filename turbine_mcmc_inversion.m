%% =========================================================================
%  涡扇发动机性能参数 MCMC 贝叶斯反演主程序
%
%  功能说明：
%    - 针对4个敏感参数（eta_k, eta_t, eta_T, lambda）进行贝叶斯反演
%    - 其余9个参数固定为已知真值
%    - 采用 Metropolis-Hastings MCMC 算法
%    - 输出后验统计量、预测对比及可视化图像
%
%  作者：[作者姓名]
%  日期：2025
...
% 敏感参数先验上下界
lb = [0.80, 0.83, 0.95, 1.00];
ub = [0.90, 0.95, 1.01, 1.06];
n_params = length(lb);
...
% 4个敏感参数真值
theta_true = [0.85, 0.89, 0.980, 1.030];
...
theta_fixed = [
    ...
    0.945, ...  % eta_c1   一次喷管效率
    0.930, ...  % eta_c2   二次喷管效率
    0.990, ...  % sigma_cc  进气道激波总压恢复
    0.985, ...  % sigma_kan 进气通道总压恢复
    0.985, ...  % sigma_kask 压气机级间总压恢复
    0.950  ...  % sigma_ks  燃烧室总压恢复
];
